...
// Sub-views
import FleetStatus from '../hospital/FleetStatus';
import TriageQueue from '../hospital/TriageQueue';
import SupplyAudit from '../hospital/SupplyAudit';
import NodeConfig from '../hospital/NodeConfig';
import RegisterIntakeModal from '../hospital/RegisterIntakeModal';

const HospitalAdminDashboard = () => {
    const { logout, user } = useAuth();
    const { notification, showNotification, hideNotification } = useDispatchNotification();
    const [activeTab, setActiveTab] = useState('Ward Flow');
    ...
    const formatIncident = (inc) => ({
        ...
        incident_type: inc.type || inc.incident_type || 'UNKNOWN',
        location_description: inc.location || inc.location_description || 'Unknown Location',
    ...
    const mapMarkers = [
        ...incidents.map(inc => ({ id: inc.id, lat: parseFloat(inc.latitude), lng: parseFloat(inc.longitude), type: 'INCIDENT', title: `Medical: ${inc.id}`, description: inc.location_description, color: 'rose', status: inc.status })),
        ...hospitalVehicles.map(veh => ({ id: veh.id, lat: parseFloat(veh.latitude), lng: parseFloat(veh.longitude), type: 'AMBULANCE', title: veh.registrationNumber, description: 'En-route to Base', color: 'teal', status: veh.status }))
    ];
    ...
    const sidebarItems = [
        { icon: 'monitor_heart', label: 'Ward Flow' },
    ...
    // ---- Ward Flow (original main content) ----
    const WardFlowView = () => (
        <>
            ...
            <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-16">
                {stats.map((stat, i) => (
                    <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} key={i} className="bg-white dark:bg-slate-900 p-8 rounded-[32px] border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-2xl transition-all relative group overflow-hidden">
                        <div className={`${stat.color} mb-6 w-14 h-14 rounded-2xl flex items-center justify-center`}>
                          ...
                          {[
                              ...
                              <div key={i} className="flex justify-between items-center">
                                  <div className="flex items-center gap-4">
                                      <div className="w-10 h-10 rounded-full bg-slate-50 dark:bg-slate-800 flex items-center justify-center font-bold text-slate-400 text-xs">{staff.name[0]}</div>
                                  ...
                                  <div className={`text-[9px] font-black px-2 py-1 rounded-[4px] ${staff.status === 'ACTIVE' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'} dark:bg-slate-800/50`}>{staff.status}</div>
                              </div>
                          ))}
                       </div>
            ...
            <div className="px-4 pb-16">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {incidents.slice(0, 6).map((inc, i) => (
                        <motion.div key={inc.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className="bg-white dark:bg-slate-900/60 p-8 rounded-[40px] border border-slate-200 dark:border-slate-800 shadow-sm group hover:border-medical-main/40 hover:bg-white dark:hover:bg-slate-900 hover:shadow-2xl transition-all relative overflow-hidden">
                            ...
                            <div className="flex flex-col gap-3 pt-6 border-t border-slate-100 dark:border-slate-800/80">
                                {inc.status === 'REPORTED' || inc.status === 'CREATED' ? (
                                    ...
                                    <div className="flex items-center justify-between">
                                        ...
                                        <button onClick={() => setActiveTab('Triage Queue')} className="flex items-center gap-2 text-xs font-black italic tracking-widest text-medical-main hover:gap-3 transition-all group">
                                            VIEW TRIAGE
    ...
    const renderContent = () => {
        switch (activeTab) {
            case 'Fleet Status': return <FleetStatus vehicles={hospitalVehicles} />;
            case 'Triage Queue': return <TriageQueue incidents={incidents} />;
            case 'Supply Audit': return <SupplyAudit />;
            case 'Node Config': return <NodeConfig />;
            default: return <WardFlowView />;
        }
    };
    ...
    return (
        <div className="flex h-screen w-full bg-background-light dark:bg-background-dark overflow-hidden">
            ...
            <RegisterIntakeModal isOpen={showIntakeModal} onClose={() => setShowIntakeModal(false)} />
        </div>
    );
};
