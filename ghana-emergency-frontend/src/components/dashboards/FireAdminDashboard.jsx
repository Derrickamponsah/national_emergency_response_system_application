    ...
    const fetchData = async () => {
        try {
            const [incidentData, vehicleData] = await Promise.allSettled([
                incidentService.getAll({ type: 'FIRE' }),
                vehicleService.getAll({ type: 'FIRE_TRUCK' })
            ]);

            if (incidentData.status === 'fulfilled') {
                setIncidents(incidentData.value.filter(inc => inc.incident_type === 'FIRE').map(formatIncident));
            }

            if (vehicleData.status === 'fulfilled') {
                setFireVehicles(vehicleData.value.filter(veh => veh.type === 'FIRE_TRUCK').map(formatVehicle));
            }
        } catch (error) {
            console.error("Fire Node Sync Error:", error);
            // Fallback mock
            setIncidents([
                { id: 5, incident_type: 'FIRE', location_description: 'Makola Market Sector B', status: 'IN_PROGRESS', latitude: 5.5451, longitude: -0.2035 },
                { id: 6, incident_type: 'FIRE', location_description: 'Agbogbloshie Node', status: 'REPORTED', latitude: 5.5503, longitude: -0.2115 }
            ].map(formatIncident));
        } finally {
            setLoading(false);
        }
        ...
        <div className="flex h-screen w-full bg-background-light dark:bg-background-dark overflow-hidden font-display">
            ...
            <aside className="w-72 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col p-6 shadow-xl z-20">
                <div className="flex items-center gap-3 mb-10 px-2 mt-4">
                    <div className="w-12 h-12 rounded-xl bg-fire-main flex items-center justify-center shadow-lg shadow-orange-500/20">
                        <span className="material-symbols-outlined text-white text-3xl">local_fire_department</span>
                    </div>
                    <div>
                        <h1 className="font-bold text-xl text-slate-900 dark:text-white leading-tight">Fire Command</h1>
                        <p className="text-[10px] font-black tracking-[0.2em] text-slate-500 uppercase italic">Makola Sector Hub</p>
                    </div>
                </div>

                <nav className="flex-1 space-y-2">
                    {[
                        { icon: 'pumping_station', label: 'Hydrant Flow', active: true },
                        ...
                        <button 
                            ...
                            className={`w-full flex items-center gap-4 px-5 py-3.5 rounded-2xl transition-all group ${
                                item.active 
                                ? 'bg-fire-main text-white shadow-xl shadow-orange-900/40 translate-x-1' 
                                : 'text-slate-500 hover:text-fire-main hover:bg-slate-50 dark:hover:bg-slate-800'
                            }`}
                        >
                ...
                <div className="mt-auto pt-6 border-t border-slate-100 dark:border-slate-800">
                    <div className="bg-slate-50 dark:bg-slate-800/50 p-5 rounded-[24px] mb-4 flex items-center gap-4 border border-slate-200/50 dark:border-slate-800">
                        <div className="w-10 h-10 rounded-full bg-fire-main/10 flex items-center justify-center text-fire-main font-bold">
                            {user?.name?.[0] || 'C'}
                        </div>
                        <div className="flex-1 overflow-hidden">
                            <p className="text-sm font-bold text-slate-900 dark:text-white truncate">{user?.name || 'Chief Lartey'}</p>
                            <p className="text-[10px] text-slate-500 uppercase font-black truncate tracking-widest">{user?.role || 'FIRE_ADMIN'}</p>
                        </div>
                    </div>
                    <button 
                        onClick={logout}
            ...
            <main className="flex-1 overflow-y-auto p-8 lg:p-12 relative custom-scrollbar bg-[radial-gradient(circle_at_top_right,_var(--tw-gradient-stops))] from-orange-50/5 via-transparent to-transparent">
                ...
                <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 mb-16 px-4">
                    <div>
                        ...
                        <div className="flex items-center gap-2 mt-3 overflow-hidden">
                           ...
                           <p className="text-slate-500 text-xs font-bold tracking-widest uppercase">Emergency Readiness Level: HIGH</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4 bg-white dark:bg-slate-900 p-2 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
                        ...
                        <button className="px-8 py-3 bg-fire-main hover:bg-orange-700 text-white rounded-xl shadow-lg shadow-orange-500/20 text-xs font-bold transition-all active:scale-95 flex items-center gap-2">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 px-4 mb-20">
                    <div className="lg:col-span-2 flex flex-col gap-6">
                        ...
                        <div className="h-[550px] w-full shadow-2xl shadow-fire-main/10 rounded-[48px] overflow-hidden border-4 border-white dark:border-slate-800">
                    ...
                    <div className="flex flex-col gap-6">
                        ...
                        <div className="bg-white dark:bg-slate-900 rounded-[48px] p-10 border border-slate-200 dark:border-slate-800 shadow-sm flex-1 space-y-10 relative overflow-hidden group">
                           <div className="absolute top-0 right-0 p-8 opacity-5 group-hover:scale-110 transition-transform">
                                <span className="material-symbols-outlined text-[100px]">pumping_station</span>
                           </div>

                           {[
                               { label: 'Makola Main', pressure: '12.4 bar', status: 'STABLE' },
                               { label: 'Independence Sq', pressure: '14.2 bar', status: 'STABLE' },
                               { label: 'Cantonments Node', pressure: '08.9 bar', status: 'LOW' },
                               { label: 'Ridge Hospital Ext', pressure: '11.1 bar', status: 'STABLE' }
                           ].map((hydrant, i) => (
                               <div key={i} className="flex justify-between items-center relative z-10 group/item">
                                   <div className="flex flex-col">
                                       <span className="text-sm font-bold text-slate-900 dark:text-white">{hydrant.label}</span>
                                       <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{hydrant.pressure}</span>
                                   </div>
                           ...
                           <button className="w-full py-5 border-t border-slate-100 dark:border-slate-800 text-[10px] font-black tracking-[0.2em] text-slate-400 hover:text-fire-main transition-all uppercase flex items-center justify-center gap-3 italic mt-12">
                               <span className="material-symbols-outlined text-lg">water_damage</span>
                               Full Hydrant Protocol
                           </button>
                        </div>
                    </div>
                </div>
