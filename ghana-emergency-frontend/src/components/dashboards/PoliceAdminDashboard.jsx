...
import PatrolMap from '../police/PatrolMap';
import SecurityLogs from '../police/SecurityLogs';
import IntelRegistry from '../police/IntelRegistry';
...
const PoliceAdminDashboard = () => {
    ...
    const [activeTab, setActiveTab] = useState('Radio Band 4');
    ...
    const fetchData = async () => {
        try {
            const [incidentData, vehicleData] = await Promise.allSettled([incidentService.getAll({ type: 'POLICE' }), vehicleService.getAll({ type: 'POLICE' })]);
            if (incidentData.status === 'fulfilled') setIncidents(incidentData.value.filter(inc => inc.incident_type === 'POLICE' || inc.incident_type === 'SECURITY').map(formatIncident));
            if (vehicleData.status === 'fulfilled') setPoliceUnits(vehicleData.value.filter(veh => veh.type === 'POLICE').map(formatVehicle));
        } catch (error) {
            console.error("Police Node Sync Error:", error);
            setIncidents([{ id: 3, incident_type: 'POLICE', location_description: 'Accra Mall Intersection', status: 'IN_PROGRESS', latitude: 5.6191, longitude: -0.1765 }, { id: 4, incident_type: 'POLICE', location_description: 'Spintex Road', status: 'REPORTED', latitude: 5.6293, longitude: -0.1585 }].map(formatIncident));
    ...
    const mapMarkers = [...incidents.map(inc => ({ id: inc.id, lat: parseFloat(inc.latitude), lng: parseFloat(inc.longitude), type: 'INCIDENT', title: `Security: ${inc.id}`, description: inc.location_description, color: 'rose', status: inc.status })), ...policeUnits.map(veh => ({ id: veh.id, lat: parseFloat(veh.latitude), lng: parseFloat(veh.longitude), type: 'POLICE', title: veh.registrationNumber, description: 'Units in Perimeter', color: 'blue', status: veh.status }))];
    const stats = [
    ...
    const sidebarItems = [
        { icon: 'radio_button_checked', label: 'Radio Band 4' },
        ...
        <>
            <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 mb-16 px-4">
                <div>
                    ...
                    <div className="flex items-center gap-2 mt-3 overflow-hidden"><span className="w-1.5 h-1.5 rounded-full bg-rose-500 animate-pulse" /><p className="text-slate-500 text-xs font-bold tracking-widest uppercase">Region Greater Accra v.3.1.2 - Sector Sync Stable</p></div>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 px-4 mb-20">
                <div className="lg:col-span-2 flex flex-col gap-6">
                    ...
                    <div className="h-[550px] w-full shadow-2xl shadow-police-main/10 rounded-[48px] overflow-hidden border-4 border-white dark:border-slate-800"><LiveMap markers={mapMarkers} className="grayscale-[0.5] contrast-125" /></div>
                </div>
                <div className="flex flex-col gap-6">
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-[48px] p-10 border border-slate-200 dark:border-slate-800 shadow-sm flex-1 space-y-8 relative overflow-hidden group">
                       <div className="absolute top-0 right-0 p-8 opacity-5 group-hover:scale-110 transition-transform"><span className="material-symbols-outlined text-[100px]">security_update_warning</span></div>
                             ...
                             <div key={i} className="flex flex-col gap-2 relative z-10">
                                 ...
                                 <div className="flex items-center gap-2 mt-1 py-2">
                                    {(inc.status === 'REPORTED' || inc.status === 'CREATED') ? (
                                        <div className="flex-1 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest flex items-center justify-center gap-2 transition-all bg-slate-100 dark:bg-slate-800 text-slate-500 italic">
                                        ...
                                        <div className="flex items-center gap-2 mt-1">
                                            ...
                                            <span className="text-[10px] text-slate-500 font-bold uppercase tracking-tighter italic">Unit Assigned & En-Route</span>
                                        </div>
                                    )}
                                 </div>
                                 {i < 3 && <div className="mt-4 border-b border-slate-50 dark:border-slate-800/50" />}
                             </div>
                         ))}
                         <button onClick={() => setActiveTab('Security Logs')} className="w-full py-5 bg-police-main text-white rounded-2xl text-[10px] font-black tracking-[0.2em] shadow-xl shadow-blue-900/40 hover:translate-y-[-2px] transition-all uppercase flex items-center justify-center gap-3 italic">
                           <span className="material-symbols-outlined text-lg">description</span>Generate Incident Report
                       </button>
                    </div>
                </div>
            </div>
    ...
    const renderContent = () => {
        switch (activeTab) {
            case 'Patrol Map': return <PatrolMap policeUnits={policeUnits} />;
            case 'Security Logs': return <SecurityLogs incidents={incidents} />;
            case 'Intel Registry': return <IntelRegistry />;
            case 'Police Config': return <PoliceConfig />;
            default: return <RadioBandView />;
        }
    };
        ...
        <div className="flex h-screen w-full bg-background-light dark:bg-background-dark overflow-hidden font-display">
            ...
            <aside className="w-72 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col p-6 shadow-xl z-20">
                <div className="flex items-center gap-3 mb-10 px-2 mt-4">
                    <div className="w-12 h-12 rounded-xl bg-police-main flex items-center justify-center shadow-lg shadow-blue-500/20"><span className="material-symbols-outlined text-white text-3xl">local_police</span></div>
                    <div><h1 className="font-bold text-xl text-slate-900 dark:text-white leading-tight">Accra Node</h1><p className="text-[10px] font-black tracking-[0.2em] text-slate-500 uppercase italic">Command Sector 1</p></div>
                </div>
                <nav className="flex-1 space-y-2">
                    {sidebarItems.map((item, i) => (
                        <button key={i} onClick={() => setActiveTab(item.label)} className={`w-full flex items-center gap-4 px-5 py-3.5 rounded-2xl transition-all group ${activeTab === item.label ? 'bg-police-main text-white shadow-xl shadow-blue-900/40 translate-x-1' : 'text-slate-500 hover:text-police-main hover:bg-slate-50 dark:hover:bg-slate-800'}`}>
                ...
                <div className="mt-auto pt-6 border-t border-slate-100 dark:border-slate-800">
                    <div className="bg-slate-50 dark:bg-slate-800/50 p-5 rounded-[24px] mb-4 flex items-center gap-4 border border-slate-200/50 dark:border-slate-800">
                        <div className="w-10 h-10 rounded-full bg-police-main/10 flex items-center justify-center text-police-main font-bold">{user?.name?.[0] || 'O'}</div>
                        <div className="flex-1 overflow-hidden"><p className="text-sm font-bold text-slate-900 dark:text-white truncate">{user?.name || 'Officer Kwame'}</p><p className="text-[10px] text-slate-500 uppercase font-black truncate tracking-widest">{user?.role || 'POLICE_ADMIN'}</p></div>
                    </div>
                    <button onClick={logout} className="w-full flex items-center justify-center gap-3 px-4 py-4 text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/10 rounded-2xl transition-all font-bold text-sm tracking-widest uppercase italic border border-rose-500/10"><span className="material-symbols-outlined text-xl">logout</span>Detach Duty</button>
                </div>
            </aside>
